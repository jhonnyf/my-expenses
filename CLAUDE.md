...
Padrão geral: Controllers finos delegam para **Services** (regra de negócio: `BudgetService`, `CategoryService`, `DashboardService`, `InvoiceService`, `IssuerService`, `PriceHistoryService`, `RecurringPurchaseService`, `ReportService`, `SearchService`) e **Actions** (operação pontual e transacional: `ImportInvoiceAction`, `FindOrCreateSocialUser`, `UpdateUserAvatarAction`, `DeleteInvoiceAction`, `MergeCategoriesAction`, `NotifyBudgetThresholdsAction`). `IssuerService` concentra todo acesso a emissor (web e API): o emissor só é listado/aberto/favoritado/apelidado se o usuário tiver ao menos uma nota dele (senão 404), e a lista só mostra emissores com nota autorizada. `BudgetService` calcula os orçamentos (limites mensais) de um mês navegável: gasto por categoria (soma dos itens) ou Geral (soma das notas, líquido de desconto), comparação com o mês anterior, projeção do mês e resumo — com orçamento Geral, os totais usam só ele (o Geral já inclui as categorias). Após `InvoiceImported`, `CheckBudgetThresholdsListener` avisa (notificação no banco) ao chegar a 80% e a 100%, uma vez por patamar e mês (`budgets.alerted_level`/`alerted_month`). Excluir uma categoria apaga o orçamento dela (`Category::booted`). `CategoryService` também dá o detalhe, os itens sem categoria, a prévia de palavras-chave e a reversão da auto-categorização; comparação com o período anterior usa `Support\Period`. `InvoiceService` concentra a lista de notas (web e API): busca, filtros, ordenação e totais — a lista mostra também notas pendentes/não confirmadas, os totais só contam as autorizadas. Duas famílias de Strategy pattern via interface + DI:
