...
const PriceHistory = (() => {
        ...
        document.getElementById('summaryCards').innerHTML = `
            <div class="kt-card">
                <div class="kt-card-content py-4 px-5">
                    ...
                    <p class="text-2xl font-bold text-green-500 mt-1">R$ ${Utils.formatCurrency(summary.min_price)}</p>
                </div>
            </div>
            <div class="kt-card">
                <div class="kt-card-content py-4 px-5">
                    <p class="text-xs text-secondary-foreground">Maior Preço</p>
                    <p class="text-2xl font-bold text-red-500 mt-1">R$ ${Utils.formatCurrency(summary.max_price)}</p>
                </div>
            </div>
            <div class="kt-card">
                <div class="kt-card-content py-4 px-5">
                    <p class="text-xs text-secondary-foreground">Preço Médio</p>
                    <p class="text-2xl font-bold text-primary mt-1">R$ ${Utils.formatCurrency(summary.avg_price)}</p>
                </div>
            </div>
            <div class="kt-card">
                <div class="kt-card-content py-4 px-5">
    ...
    const renderChart = (timeline, summary) => {
        ...
        chart.innerHTML = timeline.map(entry => {
            ...
            return `
                <div class="flex-1 flex flex-col items-center gap-1" title="${entry.issuer_name} — R$ ${Utils.formatCurrency(price)}">
                    <span class="text-xs font-mono text-secondary-foreground">R$ ${Utils.formatCurrency(price)}</span>
        ...
        tbody.innerHTML = timeline.map(entry => {
            ...
            return `
                <tr class="${rowClass}">
                    ...
                    <td class="text-right font-semibold font-mono text-sm ${priceClass}">R$ ${Utils.formatCurrency(price)}</td>
