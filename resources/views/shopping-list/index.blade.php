...
@section('content')
    ...
    <script>
        const BASE_URL = '{{ url("shopping-list") }}';
        ...
        function fetchResults(query) {
            fetch(`{{ route('shopping-list.search') }}?q=${encodeURIComponent(query)}`)
                .then(r => r.json())
                .then(data => {
                    resultsList.innerHTML = '';
                    if (data.length === 0) {
        ...
        function buildResultItem(item) {
            ...
            return `
                <div class="flex items-center justify-between px-4 py-3 hover:bg-accent/30 cursor-pointer transition-colors ${isFav ? 'bg-yellow-50 dark:bg-yellow-500/5' : ''}"
                     onclick='addToList(decodeURIComponent("${encoded}"))'>
            ...
            return `
                <div class="flex items-center justify-between py-2.5 px-4">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        <button onclick="togglePurchased(${item._index})"
                    ...
                    <div class="flex items-center gap-3 ms-4 shrink-0">
                        ${!isPurchased ? `
                            <div class="flex items-center gap-1.5">
                                <button onclick="updateQuantity(${item._index}, -1)"
                        ...
                        <button onclick="removeItem(${item._index})"
        ...
        function addSavedListToSidebar(id, name, count) {
            ...
            const html = `
                <div class="flex items-center justify-between py-2.5 px-1 group" id="saved-list-${id}">
                    <button onclick="loadList(${id})" class="flex-1 text-left min-w-0">
                    ...
                    <button onclick="deleteList(${id})" class="text-muted-foreground hover:text-destructive transition-colors ms-2 opacity-0 group-hover:opacity-100">
                        <i class="ki-filled ki-trash text-sm"></i>
                    </button>
                </div>`;
